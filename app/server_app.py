...
def json_bugs_data(bugs_data, not_found_bug_id=None):
    bugs_data[config.STORAGE_COLUMN_CONFIDENCE] = bugs_data[config.STORAGE_COLUMN_CONFIDENCE].round(config.API_CONFIDENCE_SIGNIFICANT_DIGITS)
...
def bugs_data_as_json(bugs_data, not_found_bugs = None):
    ...
    for item in bugs_data:
        component, confidence = list(sorted(item[config.STORAGE_COLUMN_PREDICTIONS].items(), key=lambda x: x[1], reverse = True))[0]
        results.append({
    ...
    try:
            ...
            # valid bug ID(s)
                ...
                if bugs_id_count == 1:
                    ...
                    if bugs_data is None or len(bugs_data) == 0:
                        status_code = 404
                        response = json_error_msg('There is no bug with ID: {}'.format(bug_id), bug_id)
                    else:
                        status_code = 200
                ...
                # bug_id list
                    ...
                    if bugs_data is None or len(bugs_data) == 0:
                        status_code = 404
                        response = json_error_msg('There are no bugs with requested IDs', list(bug_id))
                        ...
                        if bugs_count < bugs_id_count:
                            not_found_bug_id = list(bug_id - set([item[config.STORAGE_COLUMN_ID] for item in bugs_data]))
        ...
        # bug start/end dates
            ...
            if (start_date is None) or (end_date is None):
                response = json_error_msg('Invalid request, start/end dates should be {}'.format(config.API_DATE_FORMAT_MESSAGE))

            #start_date > end_date
            elif start_date > end_date:
                response = json_error_msg('Invalid request, start date should be less or equal end date')
            ...
            #start_date and end_date are valid
                ...
                if bugs_data is None or len(bugs_data) == 0:
                    status_code = 404
                    response = json_error_msg('There are no bugs between dates [{0}; {1}]'.format(start_date, end_date))
                else:
                    status_code = 200
                    response = bugs_data_as_json(bugs_data)
