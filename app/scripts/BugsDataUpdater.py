...
def checkConfig(Config):
    if cfgDownloadSection not in Config.sections():
        ...
        # States: 'Inactive', 'Busy', 'Stop'
        Config.set(cfgDownloadSection, cfgElementState, cfgValueStateInactive)
...
def adaptiveModelSelector(logger,
    ...
    ldata2 = [reduce(lambda x, y: x * y, ldata[len(ldata):n1:-1], 1)
              for n1 in range(0, len(ldata) - 1)]
            ...
            while index < nMax:
                ...
                if len(currentParams) > 0:
                    ...
                    try:
                        ...
                        logger.info(
                            'Unsupported setting parameters. Finish after first iteration.' + str(exc))
                        if index > 0:
                            break
                    try:
                    ...
                    if curScore > bestScore:
                        ...
                        logger.info('Current best model score: ' +
                                    str(bestScore) + ' with model: ' + bestModel)
...
def runBugsDataUpdater(updateIntervalDefault=3600,
    ...
    try:
        ...
        logger.warning(
            'Incorrect update interval in config. Using default update interval = ' + str(updateIntervalDefault))
    ...
    def BugsDataUpdaterFunc():
        ...
        try:
            ...
            if not os.path.exists(os.path.join(dataDir, 'bugDataTest500.csv')):
                downloadData(sName='bugDataTest500.csv', resolutions=[
                             'FIXED', 'WONTFIX'], nMax=500000, baseLogger=logger)
            ...
            if os.path.exists(downloadUntriaged(dateFrom=dtFrom, baseLogger=logger)):
                ...
                for item in products:
                    ...
                    if not os.path.exists(os.path.join(dataDir, datasetName + '.npz')):
                        makeDataset(datasetName=datasetName, data_dict=product_data,
                                    productKeys=keys_data, sDir=dataDir)
                    ...
                    if (not os.path.exists(os.path.join(modelsDir, modelName + '.json')) and
                        ...
                        logger.warning('prediction for ' + item +
                                       ' will be not calculated or saved in db')
                        continue
            ...
            writeToConfig(cfgDownloadSection, cfgElementState,
                          cfgValueStateInactive, logger=logger)
