...
def prepareData(sDir = DATA_DIR_C, sName = 'bugData.csv', maxClasses = 100, exactClasses = {},
        ...
        for key in compNames:
            ...
            if 'opendate' in prod_frame.columns:
                items = [ { 'id': x0, 'date': x6, 'data' : ' ||| '.join([x1, x2, x3, x4, x5])} 
                            ...
                            prod_frame[prod_frame[CLASS_COMPONENT_NAME] == key]['reporter'].values.tolist(),
                            prod_frame[prod_frame[CLASS_COMPONENT_NAME] == key]['short_desc'].values.tolist(),
                            prod_frame[prod_frame[CLASS_COMPONENT_NAME] == key]['op_sys'].values.tolist(),
                            prod_frame[prod_frame[CLASS_COMPONENT_NAME] == key]['rep_platform'].values.tolist(),
    ...
    if keyIndex <= maxClasses:
        if balanseVirtual:
            ...
            if otherDataCount > virtualCount:
                print('balansing...')
                product_data[VIRTUAL_CLASS_NAME] = random.sample(product_data[VIRTUAL_CLASS_NAME], virtualCount)
    ...
    if balanseBinary:
        ...
        product_data = {VIRTUAL_CLASS_NAME: product_data[VIRTUAL_CLASS_NAME], SELECTED_CLASS_NAME: selectedItems}
