...
class Logger:    
    ...
    def __init__(self, log_dir, log_file_base_name): 
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        # os.path.isdir(sDir):
        log_file_path = log_dir + '/' + log_file_base_name + '_log_' + datetime.now().strftime('%Y_%m_%d__%H_%M_%S') + '.txt'                
    ...
    def __log_message(self, message_type, message, log_to_console):
        ...
        log_message = '{message: >{width}} | '.format(message=self.log_message_number, width=MESSAGE_NUM_WIDTH)
        log_message += '{message: >{width}} | '.format(message=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                                  , width=MESSAGE_DATETIME_WIDTH)
        log_message += '{message: >{width}} | '.format(message=message_type, width=MESSAGE_TYPE_WIDTH) + message                 
