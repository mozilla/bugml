import collections, copy, os, re, sys, gc
...
class BugClassifier:
    ...
    def __init__(self, logger, classifier = SGDClassifier, classifier_parameters = {},
        ...
        self.logger = logger
        self.logger.info('Start create bug classifier: {}'.format(config.CLASSIFIER_NAMES_DICT[classifier]))        
        ...
        self.logger.info('Finish create bug classifier: {}'.format(config.CLASSIFIER_NAMES_DICT[classifier]))    
    
    def save_model(self, model_dir):
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)        
        model_file_path = model_dir + '/model_' + config.CLASSIFIER_NAMES_DICT[self.classifier].lower() + '_' + self.target_product + config.DATASET_SUFFIX + '.pkl'
    ...
    def load_model(self, model_dir, classifier):
        model_file_path = model_dir + '/model_' + config.CLASSIFIER_NAMES_DICT[classifier].lower() + '_' + self.target_product + config.DATASET_SUFFIX + '.pkl'
    ...
    def process_bugs_data(self):                        
        ...
        df.loc[~df[config.PLATFORM].isin(platform_most_common), config.PLATFORM] = config.OTHER_PLATFORMS
        ...
        reporter_most_common = utils.get_most_common(reporter_counter,
                                               min_count=config.FEATURE_VALUE_MIN_FREQUENCY)
        df.loc[~df[config.REPORTER].isin(reporter_most_common), config.REPORTER] = config.OTHER_REPORTERS
        gc.collect()
        ...
        for index, row in df.iterrows():     
            x[row_num][config.DESCRIPTION_COLUMN_NUM] = utils.clean_string(str(row[config.SUMMARY]) + ' ' + str(row[config.DESCRIPTION])).lower()
    ...
    def __make_train_test_sets(self, x, train_indices, val_indices, max_words, 
        ...
        self.logger.info('Train set size: {0}, test set size: {1}'.format(len(train_indices), len(val_indices)))        
        #SDESCRIPTION
        ...
        x_train = sp.sparse.hstack([x_train_description, x_train_platform, x_train_op_sys, x_train_reporter]).tocsr()
        x_val = sp.sparse.hstack([x_val_description, x_val_platform, x_val_op_sys, x_val_reporter]).tocsr()                   
    ...
    def __cross_validation(self, model, x, y, cross_validator, 
        ...
        for trainval_indices, test_indices in cross_validator.split(x, y):  
            ...
            gc.collect()            
            self.logger.info('{0}.Train-test set split, score: {1:.3f}'.format(iteration_num, score))
            percentage = utils.log_progress(self.logger, time_start, iteration_num, iterations_count, percentage)            
            split_number += 1
        ...
        mean_classification_report['support'] = mean_classification_report['support'].round().astype('int64')
        support = mean_classification_report['support'].sum() - mean_classification_report.at['weighted_avg / total', 'support'] 
        ...
        self.logger.info('Cross-validation classification report:\n{}'.format(utils.classification_report2string(mean_classification_report)))
    ...
    def __cross_validation_and_grid_search(self, x, y, classifier,
        ...
        iteration_num = 0
        percentage = 0
        self.logger.info('Start cross validation and model parameters grid search, iterations count: {}'.format(iterations_count))
        outer_scores = []
        outer_best_params = []
        ...
        for trainval_indices, test_indices in outer_cv.split(x, y):              
            ...
            if inner_cv_executes_count != 0:
                for parameters in parameters_grid:
                    ...
                    for train_indices, val_indices in inner_cv.split(x[trainval_indices], y[trainval_indices]): 
                        ...
                        iteration_num += 1
                        self.logger.info('{0}.iteration, score: {1:.3f}'.format(iteration_num, score), log_to_console=False)  
                        percentage = utils.og_progress(logger, time_start, iteration_num, iterations_count, percentage)                     
            ...
            iteration_num += 1
            percentage = utils.log_progress(self.logger, time_start, iteration_num, iterations_count, percentage)
            self.logger.info('{0}.outer train-test split, best score: {1:.3f}'.format(outer_split_number, score), log_to_console=False)
            self.logger.info('{0}.outer train-test split, best model parameters: {1}'.format(outer_split_number, best_params), log_to_console=False)
        ...
        for idx, score, best_params in zip(range(outer_cv.get_n_splits()), outer_scores, outer_best_params):
            ...
            self.logger.info('{0}.outer train-test split, best model parameters: {1}'.format(idx, best_params))
    ...
    def test(self, x_test, y, test_indices):
        ...
        report = utils.create_classification_report(y, test_indices, pred,
                                                    component_indices, component_names)                                        
        self.logger.info('Finish test model, score: {:.3f}\nclassification_report:\n{}\n'.format(score, report))
        gc.collect()        
        return score, report
    ...
    def load_data_and_make_model_parameters_grid_search(self, data_file_path,
        ...
        return self.__cross_validation_and_grid_search(self.x, self.y, self.classifier,
                                                ...
                                                min(grid_search_splits_count, config.GRID_SEARCH_EXECUTES_COUNT),
                                                outer_cross_validator, 
                                                min(cross_validation_splits_count, config.CROSS_VALIDATION_EXECUTES_COUNT),
