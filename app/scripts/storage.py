...
#        self.disconnect()
    ...
    def makeTableUpdater(tableName, whereValues, columnsValues):
        ...
        if isinstance(columnsValues, dict) and isinstance(whereValues, dict) and len(columnsValues) > 0:        
            ...
            if len(whereValues) > 0:
                wheres = ' WHERE ' + ' AND '.join([key + ' = ' + value for key, value in whereValues.items()])
            res += sets + '\n ' + wheres
        else: 
            raise AssertionError('makeTableUpdater require dict type for whereValues and columnsValues')
        return res
                ...
                for index, row in data_frame.iterrows():
                    ...
                    columnsValues={
                        ...
                        self.sqlWrapper(config.STORAGE_COLUMN_OPENDATE): 
                            'STR_TO_DATE(' + self.sqlWrapper(dates, '\'') + ', \'%Y-%m-%d %H:%i:%s\')',
                    ...
                    session.sql(self.makeTableInserter(tableName, columnsValues=columnsValues)).execute()
            else:
                for index, row in data_frame.iterrows():
                    if not isinstance(index, int):
                        self.logger.warning('updateTable: data_frame index must be integer')
                    ...
                    if row[config.PRODUCT] is not None and len(row[config.PRODUCT]) > 0:
                        columnsValues[self.sqlWrapper(config.STORAGE_COLUMN_PRODUCT)] = self.sqlWrapper(row[config.PRODUCT], '\'')
                    if row[config.COMPONENT] is not None and len(row[config.COMPONENT]) > 0:
                        columnsValues[self.sqlWrapper(config.STORAGE_COLUMN_COMPONENT)] = self.sqlWrapper(row[config.COMPONENT], '\'')
    ...
    def selectBugsFromTable(self, tableName = config.BUGS_TABLE_NAME_C):
        ...
        try:
            ...
            self.logger.info(str([int(item.get_string(config.STORAGE_COLUMN_ID)) for item in queryResult.fetch_all()]))
        except BaseException as exc:
    ...
    def loadUntriagedBugsFromTable(self, tableName = config.BUGS_TABLE_NAME_C):
        ...
        try:
            ...
            res[config.STORAGE_COLUMN_ID] = [int(item.get_string(config.STORAGE_COLUMN_ID)) for item in queryResult.fetch_all()]
        except BaseException as exc:
            self.logger.error('Error in loadUntriagedBugsFromTable: ' + str(exc))
        finally:
            self.closeSession(session)
    ...
    def loadPredictionsForListFromTable(self, bugsList, tableName = config.BUGS_TABLE_NAME_C):
        if not isinstance(bugsList, list) or len(bugsList) <= 0:
            self.logger.error('loadBugsPredictionsFromDB requires type list for bugsList and not empty bugsList')
            raise AssertionError('loadBugsPredictionsFromDB requires type list for bugsList and not empty bugsList')
        session = self.startSession()
        res = []
        try:
            session.sql('USE ' + self.sqlWrapper(self.database_name)).execute()
            for bugId in bugsList:
                if not isinstance(bugId, int):
                    self.logger.warning('loadPredictionsForListFromDB: bugsList must contains only integer numbers')
                    break
